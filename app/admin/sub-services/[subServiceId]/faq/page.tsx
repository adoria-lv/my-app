...
export default function SubServiceFAQAdmin() {
  ...
  const [editingFaq, setEditingFaq] = useState<SubServiceFAQ | null>(null)
  ...
  const fetchSubService = async () => {
    try {
      const response = await fetch(`/api/admin/sub-services/${subServiceId}`)
      if (response.ok) {
        const data = await response.json()
        setSubService(data)
      } else {
        toast.error('Apakšpakalpojums nav atrasts')
        router.push('/admin/sub-services')
      }
    } catch (error) {
      console.error('Error fetching sub-service:', error)
      toast.error('Kļūda ielādējot apakšpakalpojumu')
    }
  }

  const fetchFAQs = async () => {
    try {
      const response = await fetch(`/api/admin/sub-services/${subServiceId}/faq`)
      if (response.ok) {
        const data = await response.json()
        setFaqs(data)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = `/api/admin/sub-services/${subServiceId}/faq`
  ...
  const editFaq = (faq: SubServiceFAQ) => {
    setFormData({
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              FAQ: {subService?.title}
            </h1>
            <p className="text-gray-600">
              {subService?.service.title} → {subService?.title}
            </p>
            <p className="text-gray-500 text-sm">Pārvaldīt apakšpakalpojuma specifiskos FAQ jautājumus</p>
          </div>
          <button
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {faqs.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">Pievienojiet FAQ jautājumus šim apakšpakalpojumam</p>
            <button
              onClick={() => setShowForm(true)}
