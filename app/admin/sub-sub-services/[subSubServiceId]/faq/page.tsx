...
export default function SubSubServiceFAQAdmin() {
  const params = useParams()
  const router = useRouter()
  const subSubServiceId = params?.subSubServiceId as string

  const [subSubService, setSubSubService] = useState<SubSubService | null>(null)
  const [faqs, setFaqs] = useState<SubSubServiceFAQ[]>([])
  const [loading, setLoading] = useState(true)
  const [editingFaq, setEditingFaq] = useState<SubSubServiceFAQ | null>(null)
  ...
  const fetchSubSubService = async () => {
    try {
      const response = await fetch(`/api/admin/sub-sub-services/${subSubServiceId}`)
      if (response.ok) {
        const data = await response.json()
        setSubSubService(data)
      } else {
        toast.error('Apakš-apakšpakalpojums nav atrasts')
        router.push('/admin/sub-services')
      }
    } catch (error) {
      console.error('Error fetching sub-sub-service:', error)
      toast.error('Kļūda ielādējot apakš-apakšpakalpojumu')
    }
  }

  const fetchFAQs = async () => {
    try {
      const response = await fetch(`/api/admin/sub-sub-services/${subSubServiceId}/faq`)
      if (response.ok) {
        const data = await response.json()
        setFaqs(data)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = `/api/admin/sub-sub-services/${subSubServiceId}/faq`
  ...
  const editFaq = (faq: SubSubServiceFAQ) => {
    setFormData({
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              FAQ: {subSubService?.title}
            </h1>
            <p className="text-gray-600">
              {subSubService?.subService.service.title} → {subSubService?.subService.title} → {subSubService?.title}
            </p>
            <p className="text-gray-500 text-sm">Pārvaldīt apakš-apakšpakalpojuma specifiskos FAQ jautājumus</p>
          </div>
          <button
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
              ...
              {faqs.map((faq) => (
                <tr key={faq.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2 text-[#B7AB96]">
        ...
        {faqs.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">Pievienojiet FAQ jautājumus šim apakš-apakšpakalpojumam</p>
            <button
              onClick={() => setShowForm(true)}
